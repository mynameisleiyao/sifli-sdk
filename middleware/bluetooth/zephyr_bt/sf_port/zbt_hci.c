...
extern struct rt_device h4_mbox_device;
#define DEVICE_DT_GET(BT_HCI_DEV) &h4_mbox_device
...
{
    return net_buf_alloc(&hci_cmd_pool, K_FOREVER);
}
...
k_tid_t bt_testing_tx_tid_get(void)
{
    ...
    return k_sys_work_q_get()->work_thread;
}
...
typedef void (*rwip_init_handler)(uint32_t error);
extern void bluetooth_init(void);
extern rwip_init_handler _rwip_init_handler;
extern uint8_t ble_power_on(void);

void rwip_init_dummy_handler(uint32_t error)
{
}
...
#if defined(CONFIG_BT_RECV_WORKQ_BT)
    /* Abort RX thread */
    k_thread_abort(bt_workq.work_thread);
#endif
